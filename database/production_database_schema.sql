-- =====================================================
-- FinTrack Production Database Schema with Supabase Features
-- =====================================================
-- Created: 2025-12-19
-- Purpose: Production-ready database structure for Indian financial management app
-- Features: Optimized RLS policies, triggers, views, functions, indexes
-- Performance: All auth_rls_initplan and multiple_permissive_policies warnings resolved
...
CREATE TABLE IF NOT EXISTS monthly_budgets (
    ...
    alert_threshold_80 DECIMAL(5,2) DEFAULT 80.00 CHECK (alert_threshold_80 >= 0 AND alert_threshold_80 <= 100),
    alert_threshold_100 DECIMAL(5,2) DEFAULT 100.00 CHECK (alert_threshold_100 >= 0 AND alert_threshold_100 <= 100),
    is_active BOOLEAN DEFAULT true,
    created_at TIMESTAMPTZ DEFAULT NOW(),
    updated_at TIMESTAMPTZ DEFAULT NOW(),
...
-- Categories policies - Combined to avoid multiple permissive policies
...
CREATE POLICY "Users can update their own categories" ON categories
    FOR UPDATE USING (user_id = (select auth.uid()) OR (is_default = true AND user_id IS NULL));

-- Transactions policies - Optimized auth.uid() calls
...
-- Monthly budgets policies - Combined to avoid multiple permissive policies
CREATE POLICY "Users can view and manage their own monthly budgets" ON monthly_budgets
    FOR ALL USING (user_id = (select auth.uid()));

-- Annual budgets policies - Combined to avoid multiple permissive policies
CREATE POLICY "Users can view and manage their own annual budgets" ON annual_budgets
    FOR ALL USING (user_id = (select auth.uid()));

-- Financial goals policies - Combined to avoid multiple permissive policies
CREATE POLICY "Users can view and manage their own financial goals" ON financial_goals
    FOR ALL USING (user_id = (select auth.uid()));

-- Investments policies - Combined to avoid multiple permissive policies
CREATE POLICY "Users can view and manage their own investments" ON investments
    FOR ALL USING (user_id = (select auth.uid()));

-- Savings recommendations policies - Optimized auth.uid() calls
...
-- Budget performance policies - Combined to avoid multiple permissive policies
...
-- Payment methods policies - Public read/write for all authenticated users
